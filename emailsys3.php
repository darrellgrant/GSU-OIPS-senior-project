...
<?php 
	if (strlen($_POST['FileSel'])>0 || strlen($_POST['fileselcopy'])>0) {		
		...
		$filename = substr($_POST['FileSel'],0,-4) . "png";
		...
		if(isset($_POST['schedule'])) {
		    foreach($emailsplit as $e) {
		        ...
		        setCron($conn,$e,$page,$senddate,1);
		    }
		}
		...
		if(isset($_POST['sendnow'])) {
		    foreach($emailsplit as $e) {
		        $page = insertVars($conn,$page2,$e);
		        setCron($conn,$e,$page,date("Y-m-d"),0);
		    }
		}
	}
	...
	function setCron($conn,$email,$message,$senddate,$urgency) {
		$subject = 'Message from International Programs and Services';
		...
		$headers .= "From: " . $from . "\r\n" .
			"Reply-To: " . "stdyabrd@georgiasouthern.edu" . "\r\n";
			//'CC:' . "stdyabrd@georgiasouthern.edu" . "\r\n";
